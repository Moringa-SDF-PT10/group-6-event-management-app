...
const EditEventModal = ({ isOpen, onClose, eventData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const fullDateTime = dayjs(eventDate).set('hour', eventTime.hour()).set('minute', eventTime.minute()).toDate();

    const dataToSend = new FormData();
    dataToSend.append("name", name);
    dataToSend.append("description", description);
    dataToSend.append("date", fullDateTime.toISOString());
    ...
    try {
  const response = await fetch(`http://your-api-url.com/api/events/${eventData.id}`, {
    method: "PATCH",
    headers: {
      "x-auth-token": token,
    },
